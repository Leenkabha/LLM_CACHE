# policy example: `sdk-lru-policy`

Least-recently-used eviction policy. It implements the **policy** contract: it chooses which entry to evict, and in the LLM
Cache it serves as the eviction policy.
...
2. Keep `plugin.yaml` valid: change `metadata.name` and `metadata.version`, keep `spec.type: policy`.
...
4. Push to GitHub, open the LLM Cache web UI, **Plugins -> Add plugin -> GitHub repository**,
   choose type `policy`, paste the repository URL and click **Verify & Activate**.
...
Activation replays persisted entries in creation order. Recency and frequency history is not persisted, so it starts fresh. Every hit costs one (batched) network call.
