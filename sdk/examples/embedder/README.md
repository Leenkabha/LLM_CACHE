# embedder example: `sdk-hash-embedder`

Deterministic hashing embedder (a demo, not a semantic model). It implements the **embedder** contract: it turns text into a unit-length vector, and in the LLM
Cache it serves as the embedder.
...
2. Keep `plugin.yaml` valid: change `metadata.name` and `metadata.version`, keep `spec.type: embedder`.
...
4. Push to GitHub, open the LLM Cache web UI, **Plugins -> Add plugin -> GitHub repository**,
   choose type `embedder`, paste the repository URL and click **Verify & Activate**.
...
A different model name or dimension changes the vector space: activation then needs an explicit choice (flush or re-embed the cache).
