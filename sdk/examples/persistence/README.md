# persistence example: `sdk-memory-persistence`

In-memory cache-entry store (NOT durable across restarts). It implements the **persistence** contract: it stores cache entries, and in the LLM
Cache it serves as cache persistence.
...
2. Keep `plugin.yaml` valid: change `metadata.name` and `metadata.version`, keep `spec.type: persistence`.
...
4. Push to GitHub, open the LLM Cache web UI, **Plugins -> Add plugin -> GitHub repository**,
   choose type `persistence`, paste the repository URL and click **Verify & Activate**.
...
Activation needs an explicit choice when the cache has entries: migrate (copy and verify), empty, or adopt. This example loses its data when the container restarts; use a real database in production.
