# vector-store example: `sdk-memory-vector-store`

In-memory brute-force vector store. It implements the **vector-store** contract: it stores vectors and finds the nearest one, and in the LLM
Cache it serves as the vector store.
...
2. Keep `plugin.yaml` valid: change `metadata.name` and `metadata.version`, keep `spec.type: vector-store`.
...
4. Push to GitHub, open the LLM Cache web UI, **Plugins -> Add plugin -> GitHub repository**,
   choose type `vector-store`, paste the repository URL and click **Verify & Activate**.
...
200
{"matches":[{"id":"a1","distance":0.02}]}
```

## Activation notes

Activation rebuilds the candidate from persisted entries and verifies count and search behaviour first. A different metric needs a confirmed threshold.
