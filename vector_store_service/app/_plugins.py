"""Auto-discovery of drop-in plugin modules for the vector-store service.
...
Both the vector-index registry (index.py) and the similarity-metric registry
(metrics.py) call load_plugins(); the _loaded guard ensures each plugin module
is imported exactly once regardless of which registry triggers the load.
"""
